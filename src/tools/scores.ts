...
import { trimScoreboard, type TrimmedScoreboard } from "../trimmer/scores.js";
...
function filterByTeam(result: TrimmedScoreboard, team: string | undefined, resolver: Resolver, league: string): unknown {
  ...
  const filtered = result.games.filter((g) => {
    return g.home.abbreviation.toUpperCase() === abbr || g.away.abbreviation.toUpperCase() === abbr;
  });
